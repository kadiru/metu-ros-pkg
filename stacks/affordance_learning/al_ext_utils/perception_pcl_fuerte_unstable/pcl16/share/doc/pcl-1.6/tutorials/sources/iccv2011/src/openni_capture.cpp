...
OpenNICapture::snap ()
{
  if (use_trigger_)
  {
    if (!preview_)
    {
      ...
      preview_ = boost::shared_ptr<pcl16::visualization::PCLVisualizer> (new pcl16::visualization::PCLVisualizer ());

      boost::function<void (const pcl16::visualization::KeyboardEvent&)> keyboard_cb =
...
OpenNICapture::snapAndSave (const std::string & filename)
{
  ...
  if (snapped_frame)
    pcl16::io::savePCDFile (filename, *snapped_frame);
...
void
OpenNICapture::onKeyboardEvent (const pcl16::visualization::KeyboardEvent & event)
{
