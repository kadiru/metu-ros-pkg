#ifndef PCL16_TRACKING_IMPL_COHERENCE_H_
#define PCL16_TRACKING_IMPL_COHERENCE_H_
...
namespace pcl16
{
  namespace tracking
  {
    ...
    PointCloudCoherence<PointInT>::initCompute ()
    {
      ...
      {
        PCL16_ERROR ("[pcl16::%s::compute] target_input_ is empty!\n", getClassName ().c_str ());
        return false;
      }
    ...
    PointCloudCoherence<PointInT>::compute (const PointCloudInConstPtr &cloud, const IndicesConstPtr &indices, float &w)
    {
      if (!initCompute ())
      {
        PCL16_ERROR ("[pcl16::%s::compute] Init failed.\n", getClassName ().c_str ());
        return;
      }
