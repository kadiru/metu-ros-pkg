...
#define CHANNEL209 "Channel101"
#define CHANNEL210 "Channel102"
#define CHANNEL211 "Channel103"
#define CHANNEL212 "Channel104"
...
getTransformation (Vector& offset, Vector& rpy)
{
  ...
  offset[0] = cam_casis_origin[0];
  offset[1] = cam_casis_origin[1] - 0.01;
  offset[2] = cam_casis_origin[2] + 0.05;
  ...
  Matrix x_corrector (3, 3);
  Matrix y_corrector (3, 3);
  Matrix z_corrector (3, 3);

  double theta = -0.0;
  v[0] = 1.0;
  v[1] = 0.0;
  v[2] = 0.0;
  x_corrector.setCol (0, v);
  v[0] = 0.0;
  v[1] = cos (theta);
  v[2] = sin (theta);
  x_corrector.setCol (1, v);
  v[0] = 0.0;
  v[1] = -sin (theta);
  v[2] = cos (theta);
  x_corrector.setCol (2, v);

  theta = 0.07;
  v[0] = cos (theta);
  v[1] = 0.0;
  v[2] = -sin (theta);
  y_corrector.setCol (0, v);
  v[0] = 0.0;
  v[1] = 1.0;
  v[2] = 0.0;
  y_corrector.setCol (1, v);
  v[0] = sin (theta);
  v[1] = 0.0;

  v[2] = cos (theta);
  y_corrector.setCol (2, v);

  theta = -0.11;
  v[0] = cos (theta);
  v[1] = sin (theta);
  v[2] = 0.0;
  z_corrector.setCol (0, v);
  v[0] = -sin (theta);
  v[1] = cos (theta);
  v[2] = 0.0;
  z_corrector.setCol (1, v);
  v[0] = 0.0;
  v[1] = 0.0;
  v[2] = 1.0;
  z_corrector.setCol (2, v);
...
class DataPort : public BufferedPort<Bottle>
{
  ...
  onRead (Bottle& b)
  {
    ...
    else
    {
      ...
      std::cout << "translation: " << avg_tranform.offset[0] << " " << avg_tranform.offset[1] << " "
          << avg_tranform.offset[2] << std::endl;
      std::cout << "orientation: " << avg_tranform.rpy[0] << " " << avg_tranform.rpy[1] << " " << avg_tranform.rpy[2]
          << std::endl;
      tf_calculated = true;
...
{
  //ros initializations
  ros::init (argc, argv, "kinect_mocap");
  ros::NodeHandle n;
  ...
  while (n.ok ())
  {
    if (avg_tf != NULL)
    {
      br.sendTransform (tf::StampedTransform (*avg_tf, ros::Time::now (), "/base_footprint", "/right_camera_link"));
    }
    r.sleep ();
    ros::spinOnce ();
  }
